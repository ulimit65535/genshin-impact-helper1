...
class Sign(object):
    def __init__(self, cookie: str = None):
        ...
        for i in range(1, 4):
            try:
                ...
                logging.error(
                    "HTTP error when get user game roles, retry %s times ..." %
                    (i))
                ...
                logging.error(
                    "Wrong response to get user game roles, retry %s times ..." %
                    (i))
    ...
    def run(self):
        logging.info('UID is %s' %
                     (str(self._uid).replace(str(self._uid)[3:6], '***', 1)))
...
if __name__ == "__main__":
    ...
    for section in config.sections():
        ...
        if code in [0, -5003]:
            ...
            logging.info(result)
            if code == -5003:
                senddata(touser, "重复签到", result)
        else:
            logging.info(result)
            senddata(touser, "签到失败", result)
